...
export const calculateFinalPrice = (product, banners) => {
    ...
    // 2. Chuẩn hóa dữ liệu (Xử lý vụ chữ hoa/thường/null của Backend)
    ...
    const originalPrice = Number(product.gia || product.Gia || product.price || 0);

    // 3. Tìm khuyến mãi dành riêng cho MÓN (Ưu tiên số 1)
    const itemBanner = safeBanners.find(b => 
        Number(b.maMon) === itemId && Number(b.phanTramGiam) > 0
    );

    // 4. Tìm khuyến mãi dành cho DANH MỤC (Ưu tiên số 2)
    // Chỉ lấy nếu banner đó có maMon = 0 (tức là áp dụng chung)
    const categoryBanner = safeBanners.find(b => 
        Number(b.maDanhMuc) === itemCatId && Number(b.maMon) === 0 && Number(b.phanTramGiam) > 0
    );

    // 5. Chốt banner áp dụng (Món đè Danh mục)
